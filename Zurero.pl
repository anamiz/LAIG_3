...
%---------------------------------------------------------------------------------------------------------------------%
%---------TUTORIAL----------------------------------------------------------------------------------------------------%
%--Para come�ar um jogo insira um dos seguintes comandos: ------------------------------------------------------------%
...
%fun�oes necessarias para o funcionamento de move: place2 e subst%
...
%----------------------------------------------------------------------------------------------------------------------------------%
%----------------------------------------------Colocar primeira pe�a---------------------------------------------------------------%
%----------------------------------------------------------------------------------------------------------------------------------%

%primeiro a jogar � sempre o preto, da� o "1"%
first(L, N, BF) :-
                              newboard(BI), 
                              place(1, BI, L, N, BF).
...
%----------------------------------------------------------------------------------------------------------------------------------%
%----------------------------------------------MOVE-Colocar pe�as que sucedem a primeira-------------------------------------------%
%----------------------------------------------------------------------------------------------------------------------------------%
...
%-----------fun�oes necessarias para funcionamento de move--------------%             
...
%move para inserir pe�as por cima ou esquerda%

moveLeft(J, BI, It, BF):-
                              check(BI, It), !,  %caso check e true->  placeT, ou seja ha um elemento atras da pe�a%
                              placeT(J,BI, It, BF).
...
%encontra a cor da pe�a que a pedra deslizante embate%
...
%percorre a linha desejada at� chegar ao pimeiro elemento nao 0, e v� o proximo elemento a ver se nao e 0%
% se tem uma pe�a atras TRUE se nao FALSE%
...
%percorre board at� a linha desejada e retorna essa linha%
linha([H|_], 0,H).
linha([_|T], It, Res) :-
                              It1 is It-1,
                              linha(T,  It1, Res).
...
%confere se tem uma pe�a%
tempeca([0|T]) :- tempeca(T).
tempeca([1|_]):- true.
tempeca([2|_]):- true.

%confere se nao esta tudo ocupado, se estiver false%
%se estiver tudo ocupado nao e possivel inserir uma nova pe�a%
naoocupado([0|_]):-true.
naoocupado([_|T]):- naoocupado(T).

%duas pe�as no inicio da linha, a� tb nao pode inserir mais uma, nao da para empurrar%
...
%value(Board, Player, direcao,It,  Value), num pe�as repetidas * 10%
...
%choose_move(Board, Jogador, Dificuldade, MoveRes), moveres � um vetor, primeiro elemento � a direcao e segunda � a letra ou numero%
...
%encontra o elemento desejado na linha, R � o iterador aleatorio e choose diz-nos qual o valor que se enconta em Orientacao[i] ou Valores[i])%
choose([H|_], 0, H).
...
%---Fun�ao utilizada para passar os dados da funcao choose_move para a funcao move ---%
...
%----------------------Come�ar um Jogo Humano vs Humano-----------------------------------------------------------%        
hvsh :-
      write('Come�ou um jogo de Humano contra Humano'),
      nl,
      display_game1(1),
      pedir_jogada1(L,N),
      first(L, N, BN),
...
pcvspc :- 
      write('Come�ou um jogo de Computador contra Computador'),
      nl,
      display_game1(1),
      pc_primeira_jogada(L,N),
...
hvspc :-
      write('Come�ou um jogo de Humano contra PC'),
      nl,
      display_game1(1),
...
pcvsh :-
      write('Come�ou um jogo de PC vs Humano'),
      nl,
      display_game1(1),
      pc_primeira_jogada(L,N),
      write('Qual a dificuldade do primeiro jogador (Facil ou Dificil)?'),
